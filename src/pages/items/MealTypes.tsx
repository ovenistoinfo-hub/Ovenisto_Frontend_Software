...
import { Plus, Search, Pencil, Trash2, UtensilsCrossed, X, ChevronUp } from "lucide-react";
...
const MealTypes = () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader icon={<UtensilsCrossed className="h-5 w-5" />} title="Meal Types" subtitle="Manage meal service categories (Breakfast, Lunch, Dinner etc.)" actions={<Button className="gradient-primary text-primary-foreground" onClick={() => { if (showDialog) { setShowDialog(false); setEditingId(null); } else { openAdd(); } }}>{showDialog ? <><X className="h-4 w-4 mr-2" />Close Form</> : <><Plus className="h-4 w-4 mr-2" />Add Meal Type</>}</Button>} />
      {/* Inline Create / Edit Form Panel */}
      {showDialog && (
        <Card className="shadow-sm border-primary/30 bg-primary/[0.02]">
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <h3 className="text-base font-semibold">{editingId ? "Edit" : "Add"} Meal Type</h3>
