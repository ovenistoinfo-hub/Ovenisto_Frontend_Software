...
import { Plus, Search, Pencil, Trash2, SlidersHorizontal, X, ChevronUp } from "lucide-react";
...
const Modifiers = () => {
  ...
  const fetchModifiers = useCallback(async () => {
    try {
      const data = await menuService.getModifiers();
      setList(data);
    } catch (err: any) {
      toast.error(err.message || "Failed to load modifiers");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (item: ModifierRecord) => { setEditingId(item.id); setForm({ name: item.name, price: item.price, type: item.type as "addon" | "removal" }); setShowDialog(true); };

  const handleSave = async () => {
    if (!form.name.trim()) { toast.error("Modifier name is required"); return; }
    setSaving(true);
    try {
      if (editingId) {
        await menuService.updateModifier(editingId, form);
        toast.success("Updated");
      } else {
        await menuService.createModifier(form);
  ...
  return (
    <div className="space-y-6">
      <PageHeader icon={<SlidersHorizontal className="h-5 w-5" />} title="Modifiers" subtitle="Add-ons and options" actions={<Button className="gradient-primary text-primary-foreground" onClick={() => { if (showDialog) { setShowDialog(false); setEditingId(null); } else { openAdd(); } }}>{showDialog ? <><X className="h-4 w-4 mr-2" />Close Form</> : <><Plus className="h-4 w-4 mr-2" />Add Modifier</>}</Button>} />
      {/* Inline Create / Edit Form Panel */}
      {showDialog && (
        <Card className="shadow-sm border-primary/30 bg-primary/[0.02]">
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <h3 className="text-base font-semibold">{editingId ? "Edit" : "Add"} Modifier</h3>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="space-y-1.5"><Label>Modifier Name</Label><Input placeholder="Enter name" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} /></div>
              <div className="space-y-1.5"><Label>Price</Label><Input placeholder="0" type="number" value={form.price || ""} onChange={(e) => setForm((p) => ({ ...p, price: Number(e.target.value) }))} /></div>
              <div className="space-y-1.5"><Label>Type</Label><div className="flex gap-2"><Button variant={form.type === "addon" ? "default" : "outline"} size="sm" onClick={() => setForm((p) => ({ ...p, type: "addon" }))}>Addon</Button><Button variant={form.type === "removal" ? "default" : "outline"} size="sm" onClick={() => setForm((p) => ({ ...p, type: "removal" }))}>Removal</Button></div></div>
            </div>
