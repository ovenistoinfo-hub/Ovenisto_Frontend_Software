...
const IngredientUnits = () => {
  ...
  const openEdit = (item: UnitRecord) => { setEditingId(item.id); setForm({ name: item.name, status: item.status }); setShowDialog(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await inventoryService.updateUnit(editingId, form);
        toast.success("Updated");
      } else {
        await inventoryService.createUnit(form);
