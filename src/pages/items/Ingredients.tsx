...
const Ingredients = () => {
  ...
  const [form, setForm] = useState<{ name: string; brand: string; categoryId: string; unitId: string; lowStockLevel: number; purchasePrice?: number; currentStock?: number; supplierId: string; shelfLifeHours: string }>(emptyForm);
  ...
  const openEdit = (item: IngredientRecord) => {
    ...
    setForm({
      ...
      shelfLifeHours: item.shelfLifeHours != null ? String(item.shelfLifeHours) : "",
    });
    setShowDialog(true);
  };
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input placeholder="e.g. 24 (Leave blank for none)" type="number" min="0" value={form.shelfLifeHours} onChange={(e) => setForm((p) => ({ ...p, shelfLifeHours: e.target.value }))} />
              </div>
