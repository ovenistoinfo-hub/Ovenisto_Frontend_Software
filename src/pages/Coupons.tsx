    ...
    <div className="space-y-6">
      ...
      {showDialog && (
        <Card className="shadow-sm border-primary/30">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><div><Label>Valid From</Label><DatePicker value={form.validFrom || ""} onChange={v => setForm(p => ({ ...p, validFrom: v }))} /></div><div><Label>Valid To</Label><DatePicker value={form.validTo === "never" ? "" : form.validTo || ""} min={form.validFrom || ""} clearable placeholder="Never expires" onChange={v => setForm(p => ({ ...p, validTo: v || "never" }))} /></div></div>
